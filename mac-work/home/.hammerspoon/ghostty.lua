...
-- 布局模式开关：true = 每次弹出全屏铺满主显示器；false = 恢复到上次隐藏时的位置
...
local function placeOnPrimaryScreen(win)
	...
	-- restore 模式：完全不碰 frame，hide/show 本身不会改变窗口位置，
	-- 窗口天然停留在用户上次摆放的地方，只需要聚焦即可
	if not FULLSCREEN then
		win:focus()
		print("[placeOnPrimaryScreen] restore 模式，保持窗口原位置")
		return
	end
...
local function moveWindowToSpace(win, targetSpace)
	...
	if curScreen:id() ~= primaryScr:id() then
		local primarySpace = spaces.activeSpaceOnScreen(primaryScr)
...
--
-----------------------------------------------------------------
hs.alert.show("Ghostty 脚本已加载（Alt+J，" .. (FULLSCREEN and "全屏模式" or "恢复位置模式") .. "）")
