...
bright_green='\033[38;2;70;220;108m'
...
if [ -n "$native_pct" ] && [ "$native_pct" != "null" ]; then
  # v2.1.6+: use the pre-calculated percentage (most accurate)
  pct_used=$(printf "%.0f" "$native_pct")
else
  ...
  if [ "$size" -gt 0 ]; then
    pct_used=$(awk "BEGIN {printf \"%.0f\", $current * 100 / $size}")
  else
    pct_used=0
  fi
fi

[ "$pct_used" -lt 0 ] 2>/dev/null && pct_used=0
[ "$pct_used" -gt 100 ] 2>/dev/null && pct_used=100

effort=""
for settings_path in "$HOME/.claude/settings.json" "$HOME/.claude/settings.local.json"; do
...
line1+="${sep}"
line1+="${yellow}context ${pct_used}%${reset}"
line1+="${sep}"
...
line1+="${sep}"
case "$effort" in
low) line1+="${bright_green}◔ ${effort}${reset}" ;;
medium) line1+="${bright_green}◑ ${effort}${reset}" ;;
high) line1+="${bright_green}● ${effort}${reset}" ;;
xhigh) line1+="${bright_green}◉ ${effort}${reset}" ;;
max) line1+="${bright_green}✦ ${effort}${reset}" ;;
*) line1+="${bright_green}◑ ${effort}${reset}" ;;
esac
